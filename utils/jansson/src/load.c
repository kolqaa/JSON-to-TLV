...
static int lex_scan(lex_t *lex, json_error_t *error) {
    ...
    if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',')
        ...
        else if (strcmp(saved_text, "TRUE") == 0)
             lex->token = TOKEN_TRUE;
...
static char *lex_steal_string(lex_t *lex, size_t *out_len) {
    ...
    if (lex->token == TOKEN_INTEGER) {
        int sz = get_size(lex->value.integer);
        char *tmp;
        result = malloc(sizeof(char) * sz + 1);
        tmp = malloc(sizeof(char) * sz + 1);
        sprintf(tmp, "%d", (int)lex->value.integer);
        strcpy(result, tmp);
        *out_len = sz;
        free(tmp);
    }
...
static json_t *parse_object(lex_t *lex, size_t flags, json_error_t *error) {
    ...
    while (1) {
        ...
        if (lex->token != TOKEN_STRING && lex->token != TOKEN_INTEGER) {
